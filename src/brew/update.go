...
func Update() {
	...
	update, updateErr := runCommand(exec.Command, "brew", []string{"update"})
	if update != nil {
	...
	upgrade, upgradeErr := runCommand(exec.Command, "brew", []string{"upgrade"})
	if upgrade != nil {
	...
	upgradeCasks, upgradeCasksErr := runCommand(exec.Command, "brew", []string{"upgrade", "--cask"})
	if upgradeCasks != nil {
	...
	cleanup, cleanupErr := runCommand(exec.Command, "brew", []string{"cleanup"})
	if cleanup != nil {
	...
	doctor, doctorErr := runCommand(exec.Command, "brew", []string{"doctor"})
	if cleanup != nil {
